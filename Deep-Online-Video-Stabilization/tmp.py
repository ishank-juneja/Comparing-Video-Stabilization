...
# Video input paths locations
# Index wise corresponding output path locations
def make_dirs(path):
...
out_name = in_name + "_DL_stab_out.avi"
# Get input fps, use same for output
fps = int(unstable_cap.get(cv2.CAP_PROP_FPS))
# Width and Height is the hard-coded resolution in configs/v2_93.py
print("Output path is {0}".format(out_path))
videoWriter = cv2.VideoWriter(out_path, cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), OUT_FPS, (width, height))
before_frames = []
before_masks = []
after_frames = []
after_temp = []
            ...
            for ww in range(j, width):
                if (black_sum[hh + 1][ww + 1] - black_sum[hh + 1][j] - black_sum[i][ww + 1] +
                    ...
                    if s > max_s:
                        max_s = s
                        ans = [i, j, hh, ww]
cut_path = os.path.join(os.path.dirname(out_path), in_name + "_cut.avi")
print("Cut Path {}".format(cut_path))
videoWriter_cut = cv2.VideoWriter(cut_path,
