...
def get_rythm_score(vid_cap_path):
    # extract the unstable video information
    vid_cap_obj = cv2.VideoCapture(vid_cap_path);
    width = int(vid_cap_obj.get(cv2.CAP_PROP_FRAME_WIDTH))
    height = int(vid_cap_obj.get(cv2.CAP_PROP_FRAME_HEIGHT))
    length = int(vid_cap_obj.get(cv2.CAP_PROP_FRAME_COUNT))
    ...
    for i in range(sobelx.shape[0]):
        for j in range(sobelx.shape[1]):
            ...
            if sobelx[i, j] == 0:
                # print(sobelx[i, j])
                ang_unstable[i, j] = np.sign(sobely[i, j]) * np.pi / 2
                # print(ang_unstable[i, j])
