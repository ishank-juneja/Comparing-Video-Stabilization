...
for video in in_paths = ../results-new/L1/DeepStab/*_0.7.avi; do
	# Checks if a file like instance
	# actually exists or is just a glob
	[ -e "$video" ] || continue
	stable_ref_path="../dataset/DeepStab/stable/$(basename "$video" _L1_cr_0.7.avi).avi"
	unstable_ref_path="../dataset/DeepStab/unstable/$(basename "$video" _L1_cr_0.7.avi).avi"
...
#	# actually exists or is just a glob
#	[ -e "$video" ] || continue
#	stable_ref_path="../results-new/averaging-filter/DeepStab/$(basename "$video" _L1_cr_0.7.avi )_avg_stab_r_5.avi"
#	unstable_ref_path="../dataset/DeepStab/unstable/$(basename "$video" _L1_cr_0.7.avi).avi"
#	# echo -e $"Currently running on $video\n $stable_ref_path\n $unstable_ref_path\n"
#    python3 get_metrics.py -stable "$stable_ref_path" -unstable "$unstable_ref_path" -stabilized "$video"
#done
...
#	# actually exists or is just a glob
#	[ -e "$video" ] || continue
#	stable_ref_path="../results-new/averaging-filter/DeepStab/$(basename "$video" _L1_cr_0.7.avi)_avg_stab_r_25.avi"
#	unstable_ref_path="../dataset/DeepStab/unstable/$(basename "$video" _L1_cr_0.7.avi).avi"
#	# echo -e $"Currently running on $video\n $stable_ref_path\n $unstable_ref_path\n"
#    python3 get_metrics.py -stable "$stable_ref_path" -unstable "$unstable_ref_path" -stabilized "$video"
#done
