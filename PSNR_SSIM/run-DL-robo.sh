...
for video in in_paths = ../results-new/DeepLearning/big-bot-out/*_out.avi; do
	# Checks if a file like instance
	# actually exists or is just a glob
	[ -e "$video" ] || continue
	stable_ref_path="../results-new/averaging-filter/big-bot-out/$(basename "$video" _DL_stab_out.avi )_avg_stab_r_5.avi"
	unstable_ref_path="../dataset/big-bot/$(basename "$video" _DL_stab_out.avi).avi"
	#echo -e $"Currently running on $video\n $stable_ref_path\n $unstable_ref_path\n"
    python3 get_metrics-DL.py -stable "$stable_ref_path" -unstable "$unstable_ref_path" -stabilized "$video"
done
...
for video in in_paths = ../results-new/DeepLearning/big-bot-out/*_out.avi; do
	...
	[ -e "$video" ] || continue
	stable_ref_path="../results-new/averaging-filter/big-bot-out/$(basename "$video" _DL_stab_out.avi )_avg_stab_r_5.avi"
	unstable_ref_path="../dataset/big-bot/$(basename "$video" _DL_stab_out.avi).mp4"
	# echo -e $"Currently running on $video\n $stable_ref_path\n $unstable_ref_path\n"
    python3 get_metrics-DL.py -stable "$stable_ref_path" -unstable "$unstable_ref_path" -stabilized "$video"
done
...
for video in in_paths = ../results-new/DeepLearning/big-bot-out/*_out.avi; do
	...
	[ -e "$video" ] || continue
	stable_ref_path="../results-new/averaging-filter/big-bot-out/$(basename "$video" _DL_stab_out.avi )_avg_stab_r_5.avi"
	unstable_ref_path="../dataset/big-bot-clipped/$(basename "$video" _DL_stab_out.avi).avi"
	# echo -e $"Currently running on $video\n $stable_ref_path\n $unstable_ref_path\n"
    python3 get_metrics-DL.py -stable "$stable_ref_path" -unstable "$unstable_ref_path" -stabilized "$video"
done
...
for video in in_paths = ../results-new/DeepLearning/big-bot-out/*_out.avi; do
	...
	# actually exists or is just a glob
	[ -e "$video" ] || continue
	stable_ref_path="../results-new/averaging-filter/big-bot-out/$(basename "$video" _DL_stab_out.avi )_avg_stab_r_25.avi"
	unstable_ref_path="../dataset/big-bot/$(basename "$video" _DL_stab_out.avi).avi"
	# echo -e $"Currently running on $video\n $stable_ref_path\n $unstable_ref_path\n"
    python3 get_metrics-DL.py -stable "$stable_ref_path" -unstable "$unstable_ref_path" -stabilized "$video"
done
